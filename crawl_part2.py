...
# Mở và đọc file HTML
with open('text/2022_3/part2.txt', 'r', encoding='utf-8') as file:
...
TOEIC_PARTS = {
    'Part1': {
        ...
        'answerCount': 3  # Số lượng đáp án mỗi câu
    }
}
...
for i, (audio_div, transcript_div) in enumerate(zip(audio_sources,  transcripts)):
    ...
    for j in range(1):
        question_index = i * 1 + j  # Tính chỉ số câu hỏi trong toàn bộ danh sách
        ...
        question_data['answer'] = ['Statement A.', 'Statement B.', 'Statement C.']  # Mảng đáp án với 4 phần tử
