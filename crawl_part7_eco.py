...
# Mở và đọc file HTML
with open('text/2022_2/part7.txt', 'r', encoding='utf-8') as file:
    soup = BeautifulSoup(file, 'html.parser')

# Lấy tất cả các phần tử cần crawl theo class
#audio_sources = soup.find_all('div', class_='context-content context-audio')
#images = soup.find_all('div', class_='context-content text-highlightable')
details =  soup.find_all('div', class_='context-content text-highlightable')
...
for i, (question_wrapper, detail_div, transcript_div) in enumerate(zip(question_wrappers, details, transcripts)):
    #print(f"i = {i}, audio_div = {audio_div}, image_div = {image_div}, transcript_div = {transcript_div}")
    # Khởi tạo đối tượng cho từng phần tử
    obj = {'detail': '', 'transcript': '', 'questionData' : []}
    ...
    #detail
    child_div = detail_div.find('div')
    if child_div:
        detail_html = child_div.decode_contents()
        obj['detail'] = detail_html
    else:
        obj['detail'] = 'No detail found'
    ...
    arr = [2, 3, 2, 2, 3, 2, 3, 4, 4, 4, 5, 5, 5, 5, 5]
    for j in range(arr[i]):
        question_index = sum(arr[:i]) + j  # Tính chỉ số câu hỏi trong toàn bộ danh sách
